...
f=sin(0.001*pi*(t-a))+0.01*sin(60*pi*(t-a));%震源强度函数
figure;
plot(t,f);xlabel('t/s');ylabel('A');%title('震源强度函数f(t-r/c)');
grid on; 
...
figure;
plot(t(1:N-1),u);xlabel('t/s');ylabel('A');%title('位移场函数u_p(t)');
grid on;
...
plot(t(1:N-1),abs(uu));xlabel('t/s');ylabel('A');%title('远近震源分量叠加的时间域u(t)');
grid on;
figure;%subplot(3,6,18);
plot(t(1:N-1),abs(u));xlabel('t/s');ylabel('|A|');%title('位移场函数的绝对值|u_p(t)|');
grid on;

e=(abs(uu)-abs(u))./abs(u);
figure;%subplot(3,6,18);
plot(t(1:N-1),abs(e)*100);xlabel('t/s');ylabel('e/%');%title('u(t)与|u_p(t)|的绝对误差e(t)');
grid on;